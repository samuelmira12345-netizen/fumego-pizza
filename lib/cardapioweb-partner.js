...
export async function getCWPaymentMethods() {
  const res = await fetch(
    `${process.env.CW_BASE_URL}/api/partner/v1/merchant/payment_methods`,
    { headers: cwHeaders(), cache: 'no-store' }
  );
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new Error(`CW payment_methods HTTP ${res.status}: ${body}`);
  }
  return res.json();
}
...
export async function pushOrderToCW(order, items) {
  ...
  try {
    ...
    // 3. Envia o pedido
    const res = await fetch(`${process.env.CW_BASE_URL}/api/partner/v1/orders`, {
      method:  'POST',
      headers: cwHeaders(),
