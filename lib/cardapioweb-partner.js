...
export async function getCWPaymentMethods() {
  ...
  if (!res.ok) {
    throw new Error(`CW payment_methods HTTP ${res.status}: ${text.slice(0, 200)}`);
  }
  try {
    return JSON.parse(text);
  } catch {
    throw new Error(`CW payment_methods retornou conteúdo inválido (não é JSON). Verifique se CW_BASE_URL está correto.`);
  }
}
