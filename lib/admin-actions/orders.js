...
export async function handleUpdateOrder(supabase, data) {
  ...
  // Try full update; if it fails (e.g. timestamp columns not yet migrated), retry with core fields only
  const { error: updateErr } = await supabase.from('orders').update(updates).eq('id', id);
  if (updateErr) {
    const coreUpdates = {};
    if (status)                       coreUpdates.status         = status;
    if (payment_status)               coreUpdates.payment_status = payment_status;
    if (payment_method !== undefined) coreUpdates.payment_method = payment_method;
    if (fiscal_note    !== undefined) coreUpdates.fiscal_note    = fiscal_note;
    if (cash_received  !== undefined) coreUpdates.cash_received  = cash_received;
    if (payment_notes  !== undefined) coreUpdates.payment_notes  = payment_notes;
    await supabase.from('orders').update(coreUpdates).eq('id', id);
  }
