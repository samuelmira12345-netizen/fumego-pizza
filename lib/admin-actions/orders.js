...
export async function handleUpdateOrder(supabase, data) {
  const {
    ...
    delivery_person_id, delivery_street, delivery_number, delivery_complement,
    delivery_neighborhood, delivery_city, delivery_zipcode, observations,
  } = data;
  const updates = {};
  ...
  if (delivery_person_id !== undefined) updates.delivery_person_id = delivery_person_id || null;
  ...
  if (observations !== undefined) updates.observations = observations || null;
...
export async function handleUpdateOrderItems(supabase, data) {
  ...
  if (normalized.length === 0) {
    return NextResponse.json({ error: 'Informe ao menos 1 item válido.' }, { status: 400 });
  }

  const subtotal = normalized.reduce((sum, item) => sum + (item.unit_price * item.quantity), 0);
  ...
  return NextResponse.json({ success: true, subtotal, total, items: rows });
}
