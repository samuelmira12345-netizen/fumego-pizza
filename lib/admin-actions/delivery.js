...
export async function handleAssignDelivery(supabase, data) {
  ...
  // Auto-assign delivery_sort_order: append to the end of this driver's queue
  if (delivery_person_id) {
    const { data: last } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    updates.delivery_sort_order = ((last?.delivery_sort_order) || 0) + 1;
  } else {
...
export async function handleGetDeliveryQueue(supabase, data) {
  const { person_id } = data || {};
  if (!person_id) return NextResponse.json({ error: 'person_id obrigatório' }, { status: 400 });

  // Today's start in São Paulo (UTC-3)
...
export async function handleSetDeliveryPriority(supabase, data) {
