...
export async function handleGetDeliveryMetrics(supabase, data) {
  ...
  const fromInput = typeof data?.from === 'string' ? data.from.slice(0, 10) : null;
  const toInput = typeof data?.to === 'string' ? data.to.slice(0, 10) : null;

  const fallbackFrom = new Date();
  fallbackFrom.setDate(fallbackFrom.getDate() - days);

  const fromDateIso = fromInput ? `${fromInput}T00:00:00.000Z` : fallbackFrom.toISOString();
  const toDateIso = toInput ? `${toInput}T23:59:59.999Z` : null;
  ...
  const [{ data: persons, error: personsError }, { data: orders, error: ordersError }] = await Promise.all([
    ...
    supabase
      ...
      .lte('created_at', toDateIso || '9999-12-31T23:59:59.999Z'),
  ]);
