...
async function loadCatalogVisibilityOverrides(supabase) {
...
async function persistCatalogVisibilityOverrides(supabase, visibility) {
  await supabase
    .from('settings')
...
export async function handleDeleteProduct(supabase, data) {
  ...
  const visibility = await loadCatalogVisibilityOverrides(supabase);

  if (visibility.products[String(id)] !== undefined) {
    delete visibility.products[String(id)];
    await persistCatalogVisibilityOverrides(supabase, visibility);
  }

  return NextResponse.json({ success: true, catalog_visibility_overrides: visibility });
}
...
export async function handleUpdateProductFlags(supabase, data) {
  ...
  if (error) {
    // Bancos legados podem não ter is_active/is_hidden; nesse caso, o fallback em settings é suficiente
    if (error.code !== '42703') {
  ...
  return NextResponse.json({
    ...
    catalog_visibility_overrides: visibility,
  });
}
