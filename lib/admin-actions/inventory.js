...
export async function handleSaveCompoundRecipeV2(supabase, data) {
  const { id, compound_id, name, yield_quantity, items } = data;
  if (!compound_id || !name) {
  ...
  if (id) {
    await supabase.from('compound_recipes')
      .update({ name, yield_quantity: parseFloat(yield_quantity) || 1 })
      .eq('id', id);
  } else {
    const { data: inserted, error } = await supabase
      .from('compound_recipes')
      .insert({ compound_id, name, yield_quantity: parseFloat(yield_quantity) || 1 })
      .select().single();
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function handleApplyCompoundRecipe(supabase, data) {
  const { recipe_id, batches = 1 } = data || {};
  if (!recipe_id) return NextResponse.json({ error: 'recipe_id obrigatório' }, { status: 400 });
  const batchCount = Math.max(1, parseInt(batches) || 1);
  ...
  await supabase.from('stock_movements').insert({
    ...
    quantity: yieldTotal,
    reason: `Produção: ${recipe.name}`,
    notes: `${batchCount} lote(s) aplicado(s)`,
  });
