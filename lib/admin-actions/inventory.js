...
export async function handleSaveIngredient(supabase, data) {
  ...
  const extraFields = {
    correction_factor: correction_factor !== undefined ? (Number.isFinite(parseFloat(correction_factor)) ? parseFloat(correction_factor) : 0) : undefined,
...
export async function handleApplyCompoundRecipe(supabase, data) {
  ...
  const { data: compound } = await supabase
    .from('ingredients').select('current_stock, correction_factor').eq('id', recipe.compound_id).single();
  const grossYield = (parseFloat(recipe.yield_quantity) || 1) * batchCount;
  const fc = parseFloat(compound?.correction_factor) || 0;
  const netYield = fc > 0 ? +(grossYield * (1 - fc / 100)).toFixed(6) : grossYield;
  const newCompoundStock = (parseFloat(compound?.current_stock) || 0) + netYield;
