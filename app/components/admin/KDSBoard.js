...
function KitchenOrderDetailsModal({ order, items, itemsLoading, onClose }) {
  if (!order) return null;
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {itemsLoading ? (
            <p style={{ fontSize: 13, color: '#6B7280' }}>Carregando itens...</p>
...
function KitchenOrderCard({ order, items, itemsLoading, onMarkReady, onOpenDetails }) {
    ...
    <div
      ...
      {itemsLoading ? (
        <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 8, padding: '10px 12px' }}>
          <p style={{ fontSize: 13, color: '#6B7280' }}>Carregando preview...</p>
        </div>
      ) : items.length > 0 ? (
        <div style={{ background: '#F8FAFC', border: '1px solid #E5E7EB', borderRadius: 8, padding: '10px 12px' }}>
          ...
          {items.length > 3 && (
            <p style={{ fontSize: 11, color: '#6B7280', marginTop: 6, fontWeight: 600 }}>+ {items.length - 3} item(ns)</p>
          )}
        </div>
      ) : (
        <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 8, padding: '10px 12px' }}>
...
function KitchenKDS({ orders, onMarkReady, soundOn, setSoundOn, kitchenItemsByOrder, kitchenItemsLoadingByOrder, onLoadKitchenOrderItems }) {
    ...
    <div style={{ flex: 1, background: '#F8FAFC', overflowY: 'auto', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 14 }}>
              {kitchenOrders.map(o => (
                <KitchenOrderCard
                  key={o.id}
                  order={o}
                  items={kitchenItemsByOrder[o.id] || []}
                  itemsLoading={!!kitchenItemsLoadingByOrder[o.id]}
                  onMarkReady={() => onMarkReady(o.id)}
                  onOpenDetails={() => {
                    onLoadKitchenOrderItems(o.id);
                    setDetailOrder(o);
                  }}
                />
              ))}
      ...
      {detailOrder && (
        <KitchenOrderDetailsModal
          order={detailOrder}
          items={kitchenItemsByOrder[detailOrder.id] || []}
          itemsLoading={!!kitchenItemsLoadingByOrder[detailOrder.id]}
...
export default function KDSBoard({
  ...
  const [kitchenItemsByOrder, setKitchenItemsByOrder] = useState({});
  const [kitchenItemsLoadingByOrder, setKitchenItemsLoadingByOrder] = useState({});
  ...
  async function loadKitchenOrderItems(orderId) {
    if (!adminToken || !orderId) return;
    if (Object.prototype.hasOwnProperty.call(kitchenItemsByOrder, orderId) || kitchenItemsLoadingByOrder[orderId]) return;

    setKitchenItemsLoadingByOrder(prev => ({ ...prev, [orderId]: true }));
    try {
      ...
      setKitchenItemsByOrder(prev => ({ ...prev, [orderId]: d.items || [] }));
    } catch {
      setKitchenItemsByOrder(prev => ({ ...prev, [orderId]: [] }));
    } finally {
      setKitchenItemsLoadingByOrder(prev => ({ ...prev, [orderId]: false }));
    }
  }
  ...
  useEffect(() => {
    visible
      .filter(o => ['confirmed', 'preparing'].includes(o.status))
      .forEach(o => {
        if (!Object.prototype.hasOwnProperty.call(kitchenItemsByOrder, o.id) && !kitchenItemsLoadingByOrder[o.id]) {
          loadKitchenOrderItems(o.id);
        }
      });
  }, [visible, adminToken]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, background: '#F1F3F5', overflow: 'hidden' }}>
      ...
      {viewMode === 'cozinha' && (
        <KitchenKDS
          ...
          kitchenItemsByOrder={kitchenItemsByOrder}
          kitchenItemsLoadingByOrder={kitchenItemsLoadingByOrder}
          onLoadKitchenOrderItems={loadKitchenOrderItems}
        />
      )}
