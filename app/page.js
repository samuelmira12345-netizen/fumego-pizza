    ...
    <div style={{ minHeight: '100vh', background: '#1A1A1A' }}>
      ...
      <header className="header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 20, fontWeight: 'bold', color: '#D4A528' }}>FUMÊGO</h1>
        </div>
      ...
      {!storeOpen && <div style={{ background: '#E53E3E', color: '#fff', textAlign: 'center', padding: '8px', fontSize: 13 }}>🔴 Estamos fechados no momento</div>}
      ...
      {calabresa && marguerita && (
        <section style={{ padding: '24px 16px', textAlign: 'center' }}>
          <div style={{
            ...
            overflow: 'hidden', position: 'relative', border: '3px solid #D4A528',
            display: 'flex',
          }}>
            {/* Calabresa */}
            <div
              ...
              style={{
                width: '50%', height: '100%', cursor: 'pointer', position: 'relative',
                background: getImageUrl(calabresa) ? `url(${getImageUrl(calabresa)}) center/cover` : '#8B2500',
                display: 'flex', alignItems: 'flex-end', justifyContent: 'center',
              }}
            >
              <div style={{ background: 'rgba(0,0,0,0.7)', width: '100%', padding: '8px 4px', textAlign: 'center' }}>
                <p style={{ color: '#fff', fontSize: 13, fontWeight: 'bold' }}>Calabresa</p>
                <p style={{ color: '#D4A528', fontSize: 14, fontWeight: 'bold' }}>R$ {Number(calabresa.price).toFixed(2).replace('.', ',')}</p>
              </div>
            </div>

            {/* Divisor */}
            <div style={{ width: 3, background: '#D4A528', zIndex: 2 }} />
            ...
            <div
              ...
              style={{
                width: '50%', height: '100%', cursor: 'pointer', position: 'relative',
                background: getImageUrl(marguerita) ? `url(${getImageUrl(marguerita)}) center/cover` : '#228B22',
                display: 'flex', alignItems: 'flex-end', justifyContent: 'center',
              }}
            >
              <div style={{ background: 'rgba(0,0,0,0.7)', width: '100%', padding: '8px 4px', textAlign: 'center' }}>
                <p style={{ color: '#fff', fontSize: 13, fontWeight: 'bold' }}>Marguerita</p>
                <p style={{ color: '#D4A528', fontSize: 14, fontWeight: 'bold' }}>R$ {Number(marguerita.price).toFixed(2).replace('.', ',')}</p>
              </div>
            </div>
          </div>

          <p style={{ color: '#888', fontSize: 12, textTransform: 'uppercase', letterSpacing: 3, marginTop: 12, fontWeight: 'bold' }}>
            Pizza Clássica
          </p>
        </section>
      )}
      ...
      {/* ===== COMBO ===== */}
      {combo && (
        <section style={{ margin: '0 16px 16px', background: 'linear-gradient(135deg, #92702A, #D4A528)', borderRadius: 16, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 16 }}>
            ...
            <button onClick={() => openProductModal(combo)} disabled={!storeOpen}
              style={{ marginTop: 12, width: '100%', padding: '12px', background: '#1A1A1A', color: '#D4A528', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 'bold', cursor: 'pointer' }}>
              Pedir Agora →
            </button>
          </div>
        </section>
